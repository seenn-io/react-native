...
> **Why iOS 16.2?** While Live Activities were introduced in iOS 16.1, the push token API (`pushType: .token`) and `ActivityContent` struct required for remote backend updates were added in iOS 16.2. Seenn's core feature is updating Live Activities from your backend via APNs push.
